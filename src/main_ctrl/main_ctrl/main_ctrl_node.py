...
from ak_v3_driver.msg import MotorCommand, MotorState
import time
import numpy as np
...
"""This is the main control loop for the robot. Here is where we will
subscribe to joystick commands, process outputs, and then publish
wheel/leg commands.
...
class MainControlLoop(Node):
    def __init__(self):
        ...
        # -------- Safety/demo variables --------
        self.safety_on = True
        self.prev_start_button = 0  # Track previous button state for debouncing
        self.dt = 0.1  # Control loop period in seconds (10ms)
        self.demo_mode = True  # puts limiter's on speed for demos
        self.demo_power = 0.80  # capping velocity (power) at 80%
        ...
        # self.led_motor = LED(17)
        # self.led_ = LED(19)
        #
        # # button to toggle demo mode
        # self.mode_switch = Button()
        #
        # self.mode_switch.when_pressed = self.toggle_demo
        #
        ...
        # -------- ODrive variables --------
        # Mechanism:
        self.knee_gear_ratio = (
            6.0 * 30.0 / 15.0
        )  # Gear ratio for knee motors (6:1 planetary * 30:19 pulley)
    ...
    def initilize(self):
        ...
        for node_name in odrive_nodes:
            ...
            try:
                self.axis_state_clients[node_name] = self.create_client(
                    AxisState, f"/{node_name}/request_axis_state"
                )
                self.get_logger().info(f"Created service client for {node_name}")
            except Exception as e:
                self.get_logger().error(
                    f"Failed to create service client for {node_name}: {e}"
                )
        ...
        self.hip_pub = self.create_publisher(
            ControlMessage, "/hip/control_message", qos_profile
        )
        self.knee_pub = self.create_publisher(
            ControlMessage, "/knee/control_message", qos_profile
        )
        ...
        # --- CubeMars/wheels publishers and subscribers ---
        self.wheel1_sub = self.create_subscription(
            msg_type=MotorState,
            topic="/wheel1/ak_v3_driver_node/state",
            callback=self.wheel1_callback,
            qos_profile=subscriber_qos,
        )
        ...
        self.wheel2_sub = self.create_subscription(
            msg_type=MotorState,
            # FIX: this was pointed at /wheel1/... in the original file --
            # wheel2's subscriber should listen to wheel2's own state topic.
            topic="/wheel2/ak_v3_driver_node/state",
            callback=self.wheel2_callback,
            qos_profile=subscriber_qos,
        )

        self.get_logger().info("CubeMars status subscribers initialized")

        # FIX (2): these were create_subscription(...) in the original --
        # ~/cmd is something this node *sends*, so it needs a publisher.
        self.wheel1_pub = self.create_publisher(
            MotorCommand,
            "/wheel1/ak_v3_driver_node/cmd",
            qos_profile,
        )

        self.wheel2_pub = self.create_publisher(
            MotorCommand,
            "/wheel2/ak_v3_driver_node/cmd",
            qos_profile,
        )

        self.get_logger().info("CubeMars control message publishers initialized")

        # Create a timer to publish Motor commands at a regular interval:
        self.motor_timer = self.create_timer(0.015, self.publish_motor_commands)

        # Everything above succeeded -- joy_callback() is now safe to act
        # on joystick input.
        self.initialized = True

    def publish_motor_commands(self):

        self.publish_odrive_commands()

        self.publish_cubemars_commands()

    def publish_odrive_commands(self):
        # If y button is pressed, go to desired configuration
        # if (self.y_button):
        #     self.des_hip_angle = -1
        #     self.get_logger().info("Y button pressed")
        # if (self.a_button):
        #     self.des_hip_angle = 1
        # else:
        ...
        right_knee_vel = (
            self.max_knee_vel * self.right_stick_ud
        )  # + self.max_knee_vel * self.right_stick_lr
        left_knee_vel = (
            self.max_knee_vel * self.right_stick_ud
        )  # self.max_knee_vel * self.right_stick_lr
        ...
        self.knee_des_pos = (
            self.knee_des_pos + right_knee_vel * self.dt
        )  # Assuming 10ms control loop
        ...
        self.des_hip_splay = (
            self.des_hip_splay + self.bumpers * self.max_hip_vel * self.dt
        )  # Adjust splay angle based on dpad input
        self.des_hip_splay = max(
            min(self.des_hip_splay, self.max_hip_angle), self.min_hip_angle
        )  # Clamp splay angle
    ...
    def publish_cubemars_commands(self):

        # left stick for wheel control
        wheel_velocity = self.max_wheel_vel * self.left_stick_ud

        wheel_velocity = max(
            min(wheel_velocity, self.max_wheel_vel), -self.max_wheel_vel
        )

        msg = MotorCommand()

        if self.safety_on:
            # duty cycle mode to idle the motor
            msg.mode = 0
            msg.duty = 0.0
    ...
    def joy_callback(self, msg):
        ...
        if not self.initialized:
            self.get_logger().warn(
                "Motors not initialized yet, skipping joystick processing"
            )
            return
        ...
        if msg.buttons[7] == 1 and self.prev_start_button == 0:  # Rising edge detection
            ...
            self.get_logger().info(
                f"Safety mode {'enabled' if self.safety_on else 'disabled'}"
            )
            if self.safety_on:
        ...
        if not self.safety_on:
            ...
            self.right_stick_ud = self.get_curved_input(msg.axes[4])
            self.right_stick_lr = self.get_curved_input(msg.axes[3])

            # Read D-Pad for hip angle:
            self.dpad_ud = msg.axes[7]

            self.left_bumper = msg.axes[2]
            self.right_bumper = msg.axes[5]

            self.bumpers = self.get_curved_input(
                (self.left_bumper - self.right_bumper), sensitivity=0.9, power=0.05
            )

            # ---------------- Joystick Control (Wheels) ---------------
            # Here we would process the joystick command and set wheel speeds accordingly.
            self.left_stick_ud = self.get_curved_input(
                msg.axes[1]
            )  # Left stick left/right
            self.left_stick_lr = self.get_curved_input(
                msg.axes[0]
            )  # Left stick up/down
        ...
        try:
            ...
            if hasattr(self, "_service_calls_in_progress"):
                if self._service_calls_in_progress > 5:  # Limit concurrent calls
                    self.get_logger().warn(
                        f"Too many service calls in progress, skipping {node_name}"
                    )
                    return
            else:
            ...
            future.add_done_callback(
                lambda f: self.service_response_callback(f, node_name)
            )
    ...
    def service_response_callback(self, future, node_name):
        try:
            ...
            self.get_logger().debug(
                f"Service call to {node_name} completed"
            )  # Change to debug
        except Exception as e:
    ...
    def get_curved_input(self, raw_joy, sensitivity=0.75, power=None):
        """maps the raw inputs to a curve to allow more intuitive joystick control"""
        if power is None:
            # demo mode limits power for easier control and saftey
            power = self.demo_power if self.demo_mode else 1.00

        if abs(raw_joy) < 0.05:  # creates deadzone
            return 0.0

        curved_val = power * (sensitivity * (raw_joy**3)) + (
            (1 - sensitivity) * raw_joy
        )
        return curved_val
    ...
    def wheel1_callback(self, msg):
        # placeholder
        return

    def wheel2_callback(self, msg):
        # placeholder
        return
